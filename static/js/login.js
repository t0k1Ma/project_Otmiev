    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function(e) {
            ...
            fetch('/api/users/login/', {
                ...
                body: JSON.stringify({
                    username: login,  // или email, зависит от бэкенда
                    password: password
                })
            })
            ...
            .then(data => {
                if (data.token) {
                    ...
                    // Если "Запомнить меня" - сохраняем надолго
                    if (remember) {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        if (username.length < 3) {
            ...
            if (usernameExists) {
                ...
                document.getElementById('usernameError').textContent = '✓ Логин свободен';
        ...
        if (!validatePhone(phone)) {
            document.getElementById('phoneError').textContent = 'Введите корректный номер телефона (11 цифр)';
            isValid = false;
        }
